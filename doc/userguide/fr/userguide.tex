...
La première étape est de remplir les métadonnées. À partir des informations du scénario voici les métadonnées
\begin{itemize}
    \item \emph{Titre du scénario :} La nuit tous les profonds sont gris
    \item \emph{Auteur :} octopoulpe
    \item \emph{Date de création :} 23/11/2013
    \item \emph{Description :} scénarion de présentation de GM-Assistant, et d'initiation au jeu de rôle. Année de jeu 1920, à Toulouse
    \item \emph{Jeux de rôle :} L'appel de Cthulhu
    \item \emph{Joueurs :} Sylvain, Marc, Japhel
\end{itemize}
Cliquons donc sur le menu \interfaceitem{Jeu} puis \interfaceitem{Métadonnées}. La fenêtre des métadonnées (figure~\ref{metadata}) apparait :
\begin{figure}[h!]
